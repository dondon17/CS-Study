...
초기 파이프는 동기적으로 한쪽에서 읽기 또는 쓰기만 할 수 있는 일방향성의 파이프(Oridnary Pipe)이었기에, 양방향 통신을 위해는 두개의 파이프를 생성했다.  
간단하게 생성하고 사용할 수 있다는 장점이 있고, 단순한 데이터 통신을 할 때 파이프를 사용하는 것이 효율적이다. 단점으로는 전이중 통신을 위해 2개를 만들어야 하기 때문에 구현이 복잡해지고 비효율적일 수 있다.

### 6. **Named 파이프(FIFO)**

익명 파이프는 부모-자식 프로세스 처럼 통신할 프로세스를 명확히 알 수 있는 경우에 사용한다.  
반면 Named 파이프는 전혀 모르는 상태의 프로세스들 사이 통신에 사용한다. 즉, Named Pipe는 익명 파이프의 확장된 버전으로 부모 프로세스와 무관한 다른 프로세스들과의 통신을 가능하게 해주는 IPC 방식이다.
